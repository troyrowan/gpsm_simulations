...
snps %>%
  select(rs, pos, chr) %>%
  write_csv(paste0("run", run, "/RAN.genedrop.run", run, ".snps.txt"), col_names = FALSE)
...
genotypes =
  ...
  filter(id %in% ran_ped$id)


colnames(genotypes) <- c("ran_output.id", "id", "sire", "dam", "age", snps$snp)


genotypes %>%
  select(age) %>%
  write_csv(paste0("run", run, "/RAN.genedrop.run", run, ".phenotypes.txt"), col_names = FALSE)
...
seq(1,10) %>%
  ...
  write_csv(paste0("run", run, "/RAN.genedrop.run", run, ".mgf"),
            col_names = FALSE)
